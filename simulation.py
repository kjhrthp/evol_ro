...
frontLegSensorValues = numpy.zeros(num_steps)
step_index = 0
while p.isConnected() and step_index < num_steps:
    # 실시간 시뮬레이션을 사용하므로 별도의 p.stepSimulation() 호출 불필요

    # 센서 값을 읽어서 배열에 저장
    backLegSensorValues[step_index] = pyrosim.Get_Touch_Sensor_Value_For_Link("BLeg")
    frontLegSensorValues[step_index] = pyrosim.Get_Touch_Sensor_Value_For_Link("FLeg")
    step_index += 1
    time.sleep(1 / 6)  # CPU 점유율을 낮추기 위해 잠시 대기
